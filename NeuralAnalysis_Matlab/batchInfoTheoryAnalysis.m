...
for ck = 1:numrows
    if ck ~= numrows
        if INSDataTable{ck+1,7} > INSDataTable{ck,7} || isnan(INSDataTable{ck+1,7})
            ...
            disp('here')
            if strcmp(INSDataTable{ck,1},'INS2007')
                disp('check')
            end
            ...
            numPulz = INSDataTable{ck,3};
            PWs = INSDataTable{ck,4};
            ISIz = INSDataTable{ck,5};
            TrainTimes = (numPulz*PWs)+((numPulz-1)*ISIz);
            numStimBins = TrainTimes./(binsize*10^3)+2;
...
%                          end 
%                      end
                     preStim = round(0.2/binsize);
                     %medChange = mean(basal);
                     for bT = 1:12
                         curData = squeeze(dataArray{bT}{1});
                         [MIE2Vals,p] = quickEnt(curData,'nBins',20);
                         medChange = mean(MIE2Vals(1:preStim-1));
                         
                         response = MIE2Vals(preStim:round(preStim+numStimBins))-medChange;
%                          MethodAssign = {
%                          1,1,'UniCB',{200};...
%                          };
%                          StatesRaster = data2states(curData,MethodAssign);
...
%                          end
                    ...
                    MIDataCell{MIDataCellCounter,13} = curElec;
            ...
            loadFileCell = cell(1,1);
            missCounter = 1;
            catch
                disp('Did not find')
                disp(loadfile)
                loadFileCell{missCounter} = loadfile;
